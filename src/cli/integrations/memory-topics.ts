...
export const MEMORY_TOPICS: MemoryTopic[] = [
  ...
  },
  {
    ...
    description: 'For "what changed recently" or "which files did we touch", call get_activity_log — the only source of the actual file list per commit.',
    hook: 'the one "what changed" tool, and the only source of the FILES touched.',
    type: 'feedback',
    body: [
      '`get_activity_log` reads the local activity log (one entry per `commit_changes`). It replaced `get_recent_changes` / `get_developer_activity` / `get_changes_by_requirement` — all three are removed. Filters compose (AND): `developer`, `session_id`, `since_hours` or `since`/`until`, `requirement_contains`.',
      '',
      'Each entry carries `files` (every file that commit touched), `node_ids`, `developer`, `created_at`, `request`, `summary`, `status`. The response also has `all_files` — every distinct file across all matched entries, flattened into one list.',
      '',
      '**Why:** it is the answer to "show me all the files you changed" (e.g. before writing tests against recent work). The graph tools know nodes, not the full file set of a commit.',
      '',
      '**How to apply:** use it for any "what did we change recently", "what did this ticket touch", "which files from this session" question. It is LOCAL and gitignored — it reflects only this machine, never the shared graph. `devsmind activity` and `devsmind view` → Activity are the human-facing views of the same log ([[devsmind-cli-commands]]).',
    ].join('\n'),
  },
  {
