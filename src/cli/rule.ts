...
export function handleRule(opts: { path?: string }) {
  ...
  const lines = [
    ...
    `10. **Grow-as-you-go Graph Maintenance** — DevsMind is a living code graph. At the end of every task or major message where code was added, modified, renamed, or deleted, you MUST immediately update the graph. If you create new entities, call ${bt}add_node${bt} and ${bt}update_history${bt}. If you add references, call ${bt}add_connection${bt} or ${bt}remove_connection${bt}. If you notice deprecated/stale nodes or incorrect connections, clean them up immediately using ${bt}deprecate_node${bt} or ${bt}rename_node${bt} to keep the graph perfectly synchronized. Do not let the graph go stale.`,
    '',
    '### Available Tools',
    '',
    '| Tool | Use when |',
