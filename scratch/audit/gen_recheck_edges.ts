...
const TARGET_REPOS = new Set(['harrir-express-backend', 'harrir-mini-app', 'harrir-web', 'harrir-web-admin']);
...
for (const e of allEdges) {
  ...
  if (!TARGET_REPOS.has(repo)) continue;
...
const PER_REPO = 15;
const result: any = {};
let seed = 999; // different seed than before -> fresh sample
...
fs.writeFileSync('scratch/audit/recheck_edge_samples.json', JSON.stringify(result, null, 2));
