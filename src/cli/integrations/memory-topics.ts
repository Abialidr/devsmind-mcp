...
export const MEMORY_TOPICS: MemoryTopic[] = [
  ...
  },
  {
    ...
    body: [
      ...
      '',
      "**How to apply:** once bound, `commit_changes` auto-logs a step — you do NOT need `workflow_add_step` for ordinary code work. Call it for what a commit cannot express: a decision or research finding that changed no code, with the docs behind it three ways: `doc_paths` (reference — a path outside the repo is rejected because it would not exist for a teammate), `doc_uploads` (COPY up to 10 existing files, any type, into DevsMind's own storage under `.devmind/workflows/` — no repo restriction, since the point is capturing something that may not live in one, or won't stay on disk), or `doc_content` (paste up to 10 docs' full text when there's no file at all). `workflow_get_context` gives back each artifact's `file_path`, never the content, so read it from there. If you worked while unbound or on the wrong thread, `workflow_sync` attaches it afterwards from your local activity log: it previews first and only writes with `confirm:true`, and re-running is a no-op. See [[devsmind-commit-changes-contract]].",
    ].join('\n'),
  },
  {
