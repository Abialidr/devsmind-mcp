...
*   **Status note (4.0.0):** `stage_change` — the tool this phase was originally written against for non-TS/JS node creation — has been removed; `edit_node` is TS/JS-only and does not create nodes for these languages at all now (see README: "TypeScript / JavaScript projects only, for now"). This phase's actual target, `resolveConnectionsLocally()`, is unchanged by that removal and the gap below is still real — but it now presupposes a currently-nonexistent path for getting non-TS/JS nodes into the graph in the first place (previously `stage_change`, manually). Revisit this phase's scope alongside that decision before picking it back up.
