...
| **Multi-day workflows** | `workflow_create`, `workflow_bind` (per session, local to you), `workflow_list`, `workflow_get_context`, `workflow_add_step`, `workflow_remove_step` (undo a step added by mistake), `workflow_delete` (drop a workflow that never got any steps — refuses once it has history), `workflow_sync`, `workflow_archive`, `workflow_import` |
